...
const COLUMN_ALIASES: Record<string, string[]> = {
  ...
  destination2En: ['destination_2_en'],
  destinationOtherEn: ['destination_other_en'],
  airfare: ['airfare'],
...
export async function streamTravelFile(
  csvPath: string,
  sourceFileHash: string,
  onBatch: (records: TravelRecord[]) => Promise<void>,
  batchSize = 5000,
  onProgress?: (count: number) => void,
): Promise<number> {
  const encoding = detectFileEncoding(csvPath)
  console.log(`Travel CSV: detected encoding ${encoding}`)
    ...
    Papa.parse(inputStream as NodeJS.ReadableStream, {
      ...
      step: (result: Papa.ParseStepResult<string[]>, parser: Papa.Parser) => {
        ...
        batch.push({
          ...
          purposeEn: get('purposeEn'),
          destinationEn: get('destinationEn'),
          destination2En: get('destination2En'),
          destinationOtherEn: get('destinationOtherEn'),
          startDate: get('startDate'),
          endDate: get('endDate'),
          airfare: parseMoney(get('airfare')),
          otherTransport: parseMoney(get('otherTransport')),
          lodging: parseMoney(get('lodging')),
          meals: parseMoney(get('meals')),
          otherExpenses: parseMoney(get('otherExpenses')),
          total,
          normalizedName,
          sourceFileHash,
