...
import type { DeptSpendingRow } from '@/server-fns/dashboard'
...
export function DepartmentBreakdown({ data }: Props) {
  ...
  // The max pct in top-10 for normalizing the bar widths
    ...
    return (
      <div className="flex h-32 items-center justify-center rounded-lg border border-dashed">
        <p className="text-sm text-muted-foreground">
          No department data available yet. Run the aid ingestion pipeline to populate this section.
        </p>
      </div>
    )
  }
    ...
    <div className="space-y-4">
          ...
          return (
            <div key={row.department} className="space-y-1">
              <div className="flex items-center justify-between gap-2 text-sm">
                <span className="min-w-0 truncate font-medium" title={row.department}>
                  {row.department}
                </span>
              ...
              <div className="h-1.5 w-full overflow-hidden rounded-full bg-secondary">
                <div
                  ...
                  aria-label={`${row.pctOfTotal.toFixed(1)}% of total`}
                />
              </div>
            </div>
          )
      ...
      {remaining > 0 && (
        <p className="text-xs text-muted-foreground">
          and {remaining} more {remaining === 1 ? 'department' : 'departments'}
        </p>
      )}
